...
const Rotation = () => {
  ...
  const { data: rotationKeys, isPending: rotationPending } = useQuery<Rotation>({
  ...
  const { data: ChampTable, isPending: champTablePending } = useQuery<ChampTable>({
  ...
  if (champTablePending || rotationPending) {
    return <></>;
  }
